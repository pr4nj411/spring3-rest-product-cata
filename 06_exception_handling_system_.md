...
// Step 4: Handler creates structured response
ErrorResponse response = new ErrorResponse("PRODUCT_NOT_FOUND", 
    "Product with id 999 was not found.");

// Step 5: Spring converts to HTTP response
// HTTP 404 + JSON body
```
...
- **[Business Logic Service Layer](04_business_logic_service_layer_.md)**: Services throw meaningful business exceptions instead of returning error codes
- **[Data Access Repository Layer](05_data_access_repository_layer_.md)**: Repository null returns trigger appropriate business exceptions
- **[Product Domain Model](02_product_domain_model_.md)**: Exception messages reference domain concepts like "Product" and "SKU"

## Advanced Error Handling: Multiple Problems

Sometimes we might want to report multiple validation errors at once:

```java
@ExceptionHandler(InvalidProductException.class)
public ErrorResponse handleInvalid(InvalidProductException ex) {
    ErrorResponse response = new ErrorResponse("INVALID_PRODUCT", ex.getMessage());
    
    // Could add additional validation details
    if (ex.getMessage().contains("name")) {
        response.getDetails().add("Product name cannot be empty");
    }
    if (ex.getMessage().contains("price")) {
        response.getDetails().add("Price must be zero or greater");
    }
...
This allows for rich error responses that help users understand exactly what they need to fix.
...
Our exception system makes testing error scenarios straightforward:

```java
// Test that service throws correct exception
@Test
public void testProductNotFound() {
    // When repository returns null
    when(repository.findById(999L)).thenReturn(null);
    
    // Then service should throw business exception
...
The clear exception types make it easy to verify that business rules are properly enforced.

## Complete Error Flow: From Problem to Response

Let's trace a complete example where multiple validation errors occur:
...
**Validation Process:**
```java
// Service validation catches multiple problems
private void validate(ProductRequest request) {
    if (isBlank(request.getSku())) {
        throw new InvalidProductException("sku is required.");  // First problem found
    }
    if (isBlank(request.getName())) {
        throw new InvalidProductException("name is required.");  // Would be next
    }
    if (request.getPrice().compareTo(BigDecimal.ZERO) < 0) {
        throw new InvalidProductException("price must be zero or greater.");  // Would be after that
    }
}
```
...
Our current implementation reports the first problem found, which gives users clear, actionable feedback to fix their input step by step.
...
## Error Response Examples in Different Formats

Our system consistently returns structured error information:
...
{
  "code": "DUPLICATE_SKU", 
  "message": "Product SKU already exists: MOUSE-001",
  "details": []
}
```
...
Each client receives the same structured information but can format it appropriately for their users.
...
With this final piece in place, we now have a complete, robust product catalog system that handles both successful operations and error scenarios gracefully. Our Spring MVC architecture - from [controllers](03_rest_api_controller_layer_.md) through [services](04_business_logic_service_layer_.md) to [repositories](05_data_access_repository_layer_.md) - works together seamlessly to provide a reliable, user-friendly product management API.
